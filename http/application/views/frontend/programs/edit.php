<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');?>
		...
		<?php (isset($tab)) ? $tab : false;?>
			<div class="tabs__nav tabs__slider clr">
				...
				<div class="box">
					...
					<?php foreach ($tabs as $key => $tab) :?>
						<li><span class="tabs__dt tabs__dt<?php if($key == 0) { echo ' tabs__dt--active';}?>" data-tab="<?php echo $_tab_key++;?>"><?php echo $tab->name;?></span></li>
					<?php endforeach;?>
					<?php endif;?>
					</ul>
				</div>
			...
			<?php echo form_open_multipart('programs/'.$hash, array('class'=>'save-form spy', 'data-program'=>$hash)); ?>
				...
				<?php $_tab_key = 8;?>
				<?php foreach ($tabs as $key => $tab) :?>
				<div class="tabs__dd tabs__dd--<?php echo $_tab_key++;?><?php if($key == 0) { echo ' tabs__dd--active';}?>">
