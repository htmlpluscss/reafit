...
class Programs_model extends CI_Model {
    ...
    public function findPrograms($users, $user = null, $params = array(), $tags = array(), $exclude = null, $deleted = false) {
        if($users) {
            $this->db->select('e.*');
    ...
    public function getProgramsLast($user = false, $per_page = null, $page = 0, $order = array(), $count = false, $author = false, $deleted = false) {
        $this->db->select('e.*');
        $this->db->select('u.is_admin AS is_admin');
        if(!$deleted) {
            $this->db->where('e.deleted', 0);
